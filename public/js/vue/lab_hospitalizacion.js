const lab_consulta_externa = new Vue({
    ...
    mounted(){
        ...
        if(this.estado_alta == "true"){
            this.list_ecografias_data()
            console.log(" estp es lo que quiero ver")
        }  
    },
    methods:{  
        ...
        register_ecografia(e){
            ...
            fetch(this.url+'/laboratorios/vue_insert_lab_hospitalizacion/'+this.id_internacion,esto)
            .then(res => res.json())
            .catch(error => console.error('Error:', error))
            .then(data => {
                console.log(data, "   <<<<<<<<<<<<<<<<<<<<<<z")
